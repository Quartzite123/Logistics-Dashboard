...
def open_upload_dialog(triggered: bool = False) -> None:
    """Inline upload panel: pick files → preview → process → close + refresh.

    Session-state driven instead of @st.dialog so it works under stlite /
    Pyodide (which doesn't fully support the fragment-based dialog and throws
    "Could not find fragment with id" on mobile). `triggered` is the section
    header's upload-button click; once opened the panel stays visible across
    reruns until Cancel or a successful import.
    """
    if triggered:
        st.session_state["show_upload"] = True

    if not st.session_state.get("show_upload"):
        return

    with st.container(border=True):
        st.markdown("**Upload new file(s)**")
